...
if [ "$HOSTNAME" = gpu02 ]; then
  rsync -zarv --delete --delete-after --exclude="Deep.Learning/fast-style-transfer" --exclude="Deep.Learning/DeepLearningFlappyBird" \
    ...
    --include="*/" --include="*.csv" --include="*.hdf5" --include="*.pickle" --include="*.tar.gz" \
    --include="*.bin" --include="*.npz" --include="*.zip" --exclude="*" ../large-items/ .
else
